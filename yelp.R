...
## load business
buz = load.data(data.url, "yelp_academic_dataset_business",header = TRUE)
dim(buz)

colnames(buz)[1] = "business_id"
colnames(buz)[2] = "city"
colnames(buz)[3] = "business_name"
colnames(buz)[4] = "categories"
colnames(buz)[5] = "review_count"
colnames(buz)[6] = "avg_stars"
...
buz.pitt = buz.pitt[grep("Restaurant", buz.pitt$categories),]
buz.pitt.sel = subset(buz.pitt, select=-c(categories))
dim(buz.pitt.sel)
## Feature Ranking - calculate weights for each attribute using some function
weights = chi.squared(avg_stars ~ ., buz.pitt.sel)
print(weights)
top.attr = cutoff.k(weights, 50)
## print the results
f = as.simple.formula(top.attr, "avg_stars")
print(f)
## generate wordcloud
library(wordcloud)
library(stringr)
